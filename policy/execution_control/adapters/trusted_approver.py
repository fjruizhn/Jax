...
def load_trusted_approver(actor_id: str, key_id: str, *, path: str = DEFAULT_TRUSTED_APPROVERS_PATH) -> Ed25519PublicKey:
    try:
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        entry = payload["approvers"][key_id]
        if actor_id != "human:fernando" or entry["actor_id"] != actor_id: raise ValueError("actor")
