...
6. **§4.7 — cuerpo de `POST /jacobs/preflight`.** Acepta además `objective`, `user_id`, `tenant_id` opcionales (el objetivo entra al prompt; la identidad atribuye el costo de la sonda). `steps` es obligatorio con al menos 1: sin pasos, `PlanBuilder.build()` planifica con un LLM pago (`plan.py:450-453`) y un pre-vuelo no puede gastar. No mira el kill switch: no ejecuta nada.
