...
| v3.1 | (commit de esta enmienda) | **CORREGIDO 2026-09-14 (noche, Hyde, revisión final): lo que se implementó en §3.5 y §4.** (a) La recarga NO va "bajo `asyncio.Lock`" (§2, §3.5): es UNA recarga en vuelo por stamp, **compartida** (`asyncio.shield`) — los turnos que esperan reciben el mismo resultado o la misma excepción, sin reintentos en serie — y acotada por `asyncio.wait_for(GOVERNANCE_RELOAD_TIMEOUT_SECONDS)` (default 5.0 s, validada en cada recarga: finita y > 0). Motivo: con un Lock, 20 turnos con la DB caída hacían 20 intentos en serie, y una DB colgada retenía el lock sin límite (revisión de la Tarea 6). (b) Fuera del alcance original y agregado por el mismo motivo: **`connect_timeout` de config en todo `aiomysql.connect`/`create_pool`** de jax y jax-platform (`JAX_DB_CONNECT_TIMEOUT_SECONDS`, default 10 s; aiomysql 0.3.2 no tenía límite), con helper espejo `db_connect_config` y tripwires AST. Solo acota el socket TCP; la recarga completa la acota (a). |
