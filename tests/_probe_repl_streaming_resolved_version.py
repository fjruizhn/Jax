...
async def _reset_baseline(conn):
    async with conn.cursor() as cur:
        ...
        # contaminado: gpt-5.5-DRIFTED-BY-PROBE, id 307, huérfano). Primero
        # cualquier proposal que la referencie (FK RESTRICT, no CASCADE:
        # borrar `model` antes revienta), después la fila de `model` misma.
        # LIKE, no el nombre exacto: el modelo real de la faceta puede
