...
Ahora salen de JAX_MEMORY_EMBED_MODEL / _DIM / _COLUMN. El corte en
producción se hizo el 2026-09-12 por configuración; desde entonces los
defaults son los de producción (bge-m3), y volver atrás es fijar las tres
variables a los valores de nomic -- quitarlas ya no revierte nada.
...
class CargarConfigTest(unittest.TestCase):
    def test_sin_variables_quedan_los_valores_de_produccion(self):
        c = cfgmod.cargar({})
    ...
    def test_lee_modelo_dimension_y_columna_del_entorno(self):
        # Valores DISTINTOS de los defaults: con los de bge-m3 este test pasaría
        # aunque cargar() ignorara el entorno.
        c = cfgmod.cargar({
    ...
    def test_revertir_protege_la_columna_que_resuelve_la_config(self):
        # `revertir` se niega a borrar la columna que usan los servicios. Si la
        # resolviera con un literal y no con embedding_config, al cambiar los
        # defaults quedaría desprotegida justo cuando la variable falta del
        # entorno: los servicios usarían una columna y el script creería otra.
        for env in ({}, {"JAX_MEMORY_EMBED_COLUMN": "embedding"},
                    {"JAX_MEMORY_EMBED_COLUMN": "embedding_bge_m3"}):
            with self.subTest(env=env):
                self.assertEqual(mig.columna_activa(env), cfgmod.cargar(env).column)
