...
- **Anotado — límite de activos entre procesos en `MAX_PARALLEL_PIPELINES` (2026-09-17, plan prevuelo-y-continuar, desvío 7). jax; dueño: próxima ronda de pago de deuda.** `jacobs/continuar.py` toma `candado_de_creacion` (un `asyncio.Lock` de proceso) para el continue. El CLI `tools/jacobs_relaunch.py` corre en OTRO proceso: la transacción con `FOR UPDATE` y la época impiden un doble continue del MISMO pipeline, pero el conteo contra `MAX_PARALLEL_PIPELINES` hecho desde el CLI tiene carrera con un `create` simultáneo de LAS MANOS — cada proceso cuenta y decide con su propio candado, ninguno ve la reserva del otro. Arreglo: contar y reservar el cupo dentro de la misma transacción SQL (no en memoria de proceso), como ya hace el continue con `FOR UPDATE`. **Fecha de control: 2026-10-15.**

- **Anotado — la sonda del pre-vuelo que vence sin registrar uso (2026-09-17, plan prevuelo-y-continuar + Task 7 fix round 1, Ruling R14). jax; dueño: Task 14 de este mismo plan.** `jacobs/sonda.py::_registrar` solo llama a `record_direct_usage` cuando `r.tokens_in or r.tokens_out` es verdadero (comentario en el sitio de la llamada). Dos caminos dejan la llamada sin fila en `axioma_usage` aunque el proveedor haya podido cobrarla: (1) **timeout de la sonda** — la llamada no termina, así que `ResultadoSonda` no conoce tokens; y (2) **un 2xx del proveedor SIN campo de usage** (`usage`/`usageMetadata`/`prompt_eval_count`+`eval_count` ausentes o en cero), que deja `ResultadoSonda(tokens_in=0, tokens_out=0)`. En los dos casos ese costo real no aparece en Admin → Costos. Testeado que una sonda FALLIDA (provider_error/config_error/timeout) no registra uso (`tests/test_prevuelo_sonda.py::test_una_sonda_fallida_no_registra_uso`) — cubierto a propósito hasta el arreglo; el caso que falta es el 2xx-sin-usage. Arreglo propuesto para los dos: registrar la llamada con tokens estimados (el techo `sonda_max_tokens()` como cota) y una marca de estimación. Se reabre si algún proveedor de la ronda de facetas vigente responde así en la práctica (verificar contra `las_manos/logs/motor_jobs.jsonl`). **Fecha de control: 2026-10-15**, o antes si la Task 14 de este plan lo cierra primero.
