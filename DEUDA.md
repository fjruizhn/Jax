...
- **Anotado — la sonda del pre-vuelo que vence sin registrar uso (2026-09-17, Task 7 fix round 1, Ruling R14). jax; dueño: Task 14 de este mismo plan.** `jacobs/sonda.py::_registrar` solo llama a `record_direct_usage` cuando `r.tokens_in or r.tokens_out` es verdadero (comentario en el sitio de la llamada). Caso pendiente: **un 2xx del proveedor SIN campo de usage** (`usage`/`usageMetadata`/`prompt_eval_count`+`eval_count` ausentes o en cero) deja `ResultadoSonda(tokens_in=0, tokens_out=0)` — la sonda se pagó al proveedor pero queda **sin fila en `axioma_usage`**: ese costo real no aparece en Admin → Costos. Testeado que una sonda FALLIDA (provider_error/config_error/timeout) no registra uso (`tests/test_prevuelo_sonda.py::test_una_sonda_fallida_no_registra_uso`) — ese camino está cubierto a propósito, el que falta es el 2xx-sin-usage. Se reabre si algún proveedor de la ronda de facetas vigente responde así en la práctica (verificar contra `las_manos/logs/motor_jobs.jsonl`). **Fecha de control: 2026-10-15**, o antes si la Task 14 de este plan lo cierra primero.
