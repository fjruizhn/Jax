...
def main():
    ...
    OUTPUT.write_text(f'''<!doctype html><html><head><meta charset="utf-8"><style>@page {{ size: Letter; margin: 0.6in; }} body {{ font-family: serif; }} .notice {{ border: 2px solid #900; padding: .4em; font-weight:bold; }}</style></head><body><p class="notice">NON-AUTHORITATIVE GENERATED PRESENTATION</p><p>Source: docs/operations/operational-manual.md<br>Generator: scripts/generate_operational_manual.py<br>Revision: {revision()}</p><p>Query jaxctl and docs/operations/query-guide.md for current runtime state.</p>{body}</body></html>''',encoding='utf-8')
